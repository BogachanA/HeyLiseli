...
def internships(request, letter):
    allCategories = IntCategory.objects.all()  # TODO using queryset / SQL Impolite
    allInts = Internship.objects.all()
    topInts = allInts.order_by('-view_count')[:10]
    valid_letters = []

    # TODO save first letter on owned model and select distinct these values as alphabet
    # TODO i.e. Internship.objects.values_list('first_letter', flat=True).distinct() values/values_list(flat) <- group by
    ...
    if letter in exception_letters:
        filtered_ints = allInts.filter(name__istartswith=letter) | allInts.filter(
...
def internship_detail(request, internship_id):  # TODO add recommended ints by helper functions
    """
    returns internship detail
    :param request: given request
    :param internship_id: sdas
    :return: sadsad
    """  # TODO use more definitions
    ...
    if request.user.is_authenticated():
        ...
        if request.method == "GET":
            apply_form = applyToInternship(
                {'name': user.first_name, 'surname': user.last_name, 'school': user.lise.name, 'grade': user.grade,
                 'mail': user.email})
    else:
