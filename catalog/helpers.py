...
def find_favorite_int_category(user):
    id=user.id
    fav_categories={}
    intviews_of_user=Int_View.objects.filter(session_key_or_user_id=id).select_related('rel_int__category').values() #TODO Fix new way
...
def find_recommended_ints(target_int, user): #TODO [ASK] userı paramater olarak view de mi sunmalı yoksa liseli manytomany model field yapıp giriş yapınca update mi etmeli
    ...
    for internship in all_ints:
        ...
        if category == curr_category:
            same_category_point = 5
        else:
            same_category_point = 0
